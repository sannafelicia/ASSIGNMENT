...
public class BasicBehaviour : MonoBehaviour
{
	...
	public float raySubtractModification = 0.0f;		  // Removes this value from the *length* of the ray
	...
	public bool IsGrounded()
	{
		...
		Vector3 origin = capsuleCollider.transform.position + Vector3.up * (capsuleCollider.height * 0.5f);
		float rayLength =  (capsuleCollider.height * 0.5f - raySubtractModification - capsuleCollider.radius);
		
		Ray ray = new Ray(origin, Vector3.down);
		
        return Physics.SphereCast(ray, capsuleCollider.radius, rayLength);
	...
	private void OnDrawGizmos()
	{
		...
		Vector3 rayEnd = origin + Vector3.down * (capsuleCollider.height * 0.5f - raySubtractModification);
